...
/* Hide the Gradio footer (removes 'Built with Gradio' and 'Use via API') */
...
def prepare_all_models(voxcpm_id: str, zip_id: str, asr_id: str, cache_dir: str = "./models"):
    ...
    zip_path = snapshot_download(zip_id, cache_dir=cache_dir)
    asr_path = snapshot_download(asr_id, cache_dir=cache_dir)
    return vox_path, zip_path, asr_path
...
class VoxCPMDemo:
    def __init__(self, model_id: str = "OpenBMB/VoxCPM2", device: str = "auto") -> None:
        ...
        # Enable torch.compile for faster inference (requires nvcc in environment)
        self.optimize = True
        ...
        # 统一预下载，获取本地绝对路径
        self.local_vox_path, self.local_zip_path, self.local_asr_path = prepare_all_models(
            model_id, self.zip_model_id, self.asr_model_id
        )

    def get_or_load_voxcpm(self) -> voxcpm.VoxCPM:
        if self.voxcpm_model is not None:
            return self.voxcpm_model
        logger.info(f"Loading VoxCPM model from: {self.local_vox_path}")
        self.voxcpm_model = voxcpm.VoxCPM.from_pretrained(
            hf_model_id=self.local_vox_path,
            zipenhancer_model_id=self.local_zip_path,
    ...
    def get_or_load_asr_model(self) -> AutoModel:
        ...
        logger.info(f"Loading ASR model from: {self.local_asr_path}")

        self.asr_model = AutoModel(
            model=self.local_asr_path,
            disable_update=True,
            log_level="DEBUG",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="auto",
        help="Runtime device: auto, cuda, or cuda:N (default: auto)",
    )
